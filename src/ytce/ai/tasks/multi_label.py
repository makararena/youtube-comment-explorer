"""
Multi-label classification task executor.
"""
...
def execute_task(
    ...
    """
    Execute multi-label classification task.
    
    Args:
        task: TaskConfig with type=MULTI_LABEL
    ...
    Raises:
        ValueError: If task type is not MULTI_LABEL
        TaskExecutionError: If execution fails
    """
    if task.type.value != "multi_label":
        raise ValueError(
            f"Task type mismatch: expected multi_label, got {task.type.value}"
        )
