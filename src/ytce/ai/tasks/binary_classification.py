"""
Binary classification task executor.
"""
...
def execute_task(
    ...
    """
    Execute binary classification task.
    
    Args:
        task: TaskConfig with type=BINARY_CLASSIFICATION
    ...
    Raises:
        ValueError: If task type is not BINARY_CLASSIFICATION
        TaskExecutionError: If execution fails
    """
    if task.type.value != "binary_classification":
        raise ValueError(
            f"Task type mismatch: expected binary_classification, got {task.type.value}"
        )
