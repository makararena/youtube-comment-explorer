...
def execute_task(
    ...
    Args:
        task: TaskConfig with type=SCORING
        comments: List of comments to score
    ...
    Raises:
        ValueError: If task type is not SCORING
        TaskExecutionError: If execution fails
    """
    if task.type.value != "scoring":
        raise ValueError(
            f"Task type mismatch: expected scoring, got {task.type.value}"
        )
